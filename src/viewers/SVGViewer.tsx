...
const SVGGrid = ({ viewbox }: { viewbox: any }) => {
...
const stringifyViewbox = ({ xMin, yMin, xMax, yMax }: any) => {
  return [
    xMin.toFixed(2),
    yMin.toFixed(2),
    (xMax - xMin).toFixed(2),
...
const dashArray = (strokeType: string | undefined) => {
...
const ShapePath = ({ shape }: { shape: any }) => {
  return (
    <path
...
const addMarginToViewbox = (viewbox: any, margin: number) => {
  const { xMin, yMin, xMax, yMax, width, height } = viewbox;
  return {
...
const SVGWindow = ({ viewbox, withGrid, defaultColor, children }: any) => {
  const [clientRect, setRect] = useState<DOMRect | null>(null);
...
export default function SVGViewer({
  ...
  if (shape && shape.length && shape[0].format === "svg") {
    const viewbox = mergeViewboxes(shape.map((s: any) => s.viewbox));
    return (
